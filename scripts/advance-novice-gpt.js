...
document.addEventListener("DOMContentLoaded", () => {
  ...
  let currentRuleKeyGirls = null; // checking correction
...
// Script for the Free Program Girls
document.addEventListener("DOMContentLoaded", () => {
  // Dynamically generate buttons
  const fpGirlsButtonsHTML = Object.keys(advanceNoviceRules.fp)
    .map(key => `<button class="main-buttons" data-rule-key="${key}">${key.replace(/([A-Z])/g, ' $1')}</button>`)
    .join('');
  ...
  function toggleRuleContent(ruleKey) {
    ...
    // Toggle content: clear if the current content matches the selected rule
    const ruleValue = advanceNoviceRules.fp[ruleKey];
...
document.addEventListener("DOMContentLoaded", () => {
  // Dynamically generate buttons
  const fpBoysButtonsHTML = Object.keys(advanceNoviceRules.fp)
    .map(key => `<button class="main-buttons" data-rule-key="${key}">${key.replace(/([A-Z])/g, ' $1')}</button>`)
    .join('');
